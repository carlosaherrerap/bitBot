...
class CommandHandler {
    ...
    async handle(msg, sock) {
        ...
        console.log(`[COMMAND] Received: "${text}" from ${remoteJid}`);
        ...
        if (this.awaitingMod) {
            ...
            console.log(`[MOD] Updating ${variable} in ${script}`);
            ...
            try {
                ...
                console.log(`[MOD] Success: ${variable} updated.`);
            } catch (err) {
        ...
        try {
            if (text.startsWith('list ')) {
                ...
                console.log(`[CACHE] Saving current path: ${stateManager.getCurrentPath()}`);
                stateManager.setCachePath(stateManager.getCurrentPath());
                await reply(`📍 Ruta guardada en caché: ${stateManager.getCachePath()}`);
            }
            else if (text === 'atras') {
                const oldPath = stateManager.getCurrentPath();
                stateManager.moveUp();
                const newPath = stateManager.getCurrentPath();
                console.log(`[BACK] Navigating up from ${oldPath} to ${newPath}`);
                if (oldPath === newPath) {
                    await reply(`⚠️ Ya estás en la raíz: ${newPath}. Para cambiar de disco usa "list E:/"`);
                } else {
                    await reply(`🔙 Nueva ruta: ${newPath}`);
                }
            }
            else if (text === 'opc') {
                console.log('[OPC] Showing command list');
                this.opcIndex = 0;
                ...
                const targetDir = stateManager.getCachePath() || stateManager.getCurrentPath();
                const fullPath = path.join(targetDir, name);
                console.log(`[CREATE] Attempting to create dir: ${fullPath}`);
                ...
                const targetDir = stateManager.getCachePath() || stateManager.getCurrentPath();
                const fullPath = path.join(targetDir, name);
                console.log(`[REMOVE] Attempting to remove: ${fullPath}`);
                ...
                const filePath = path.join(stateManager.getCurrentPath(), script);
                console.log(`[EXEC] Attempting to run script: ${script}`);
                if (await fs.pathExists(filePath)) {
                    ...
                    console.log(`[EXEC] Started script ${script}`);
                } else {
                    await reply(`❌ Script no encontrado: ${filePath}`);
                    console.error(`[EXEC] Script not found: ${filePath}`);
                }
            }
            else if (text.startsWith('mod ')) {
                const script = text.replace('mod ', '').trim();
                const filePath = path.join(stateManager.getCurrentPath(), script);
                console.log(`[MOD] Request variables for: ${script}`);
                ...
                if (vars.length > 0) {
                    ...
                    console.warn(`[MOD] No editable variables in ${script}`);
                }
            }
            else if (text === 'estado') {
                console.log('[STATUS] Checking script states');
                ...
                console.log(`[LOGS] Fetching logs for: ${script}`);
                const logs = scriptRunner.getLogs(script);
                await reply(`📄 LOGS [${script}]:\n\n${logs}`);
            }
            else if (text.startsWith('cut ')) {
                const name = text.replace('cut ', '').trim();
                const fullPath = path.join(stateManager.getCurrentPath(), name);
                stateManager.setCopyBuffer(fullPath, 'cut');
                await reply(`✂️ Cortado: ${name}`);
                console.log(`[CUT] Buffer set: ${fullPath}`);
            }
            else if (text.startsWith('copy ')) {
                const name = text.replace('copy ', '').trim();
                const fullPath = path.join(stateManager.getCurrentPath(), name);
                stateManager.setCopyBuffer(fullPath, 'copy');
                await reply(`📋 Copiado: ${name}`);
                console.log(`[COPY] Buffer set: ${fullPath}`);
            }
            else if (text.startsWith('paste ')) {
                console.log('[PASTE] Processing buffer');
                ...
                if (last) {
                    console.log(`[NOW] Fetching last process logs: ${last}`);
                ...
                console.log(`[REPORT] Attempting to send: ${reportPath}`);
            ...
            await reply(`❌ Error crítico al ejecutar comando: ${err.message}`);
        }
    }
